    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('khoa-hoc.update', ['id' => $khoahoc->id]) }}" method="POST">
                ...
                <div class="mb-3">
                    <label for="mo_ta" class="form-label">Tên khoa <label style="color: red;"
                            for="mo_ta">*</label></label>
                    <input type="text" class="form-control" id="mo_ta" value="{{ $khoahoc->mo_ta }}" name="mo_ta" placeholder="Nhập tên khoa"
                        required autocomplete="off">
                    @error('mo_ta')
                        <span class="form-text">{{ $message }}.</span>
                    @enderror
